...
async function copyCustomize(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const cus = await customizeService.copyCustomizeService(customId, userId);
    if (cus == null) {
...
async function shareCustomize(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const check = await customizeService.getCustomizeByIdService(id);
    if (check == null) {
      respone(res, null, "Customize id not found !", 404);
      return;
    }
    if (check.share == true) {
      respone(res, null, "Customize is shared !", 409);
      return;
    }
...
async function updateCustomize(
  ...
  try {
    ...
    if (
      ...
      respone(res, null, "bad request filed", 400);
      return;
    }
...
async function getRandom(req: Request, res: Response, next: NextFunction) {
  try {
    const { id } = req.params;
    ...
    if (type == null) {
      respone(res, null, "type must not null", 400);
      return;
    }
    const customize = await customizeService.getRandomService(id, type);
    respone(res, customize, "Get random component Successfully", 200);
  } catch (error) {
    respone(res, null, `${error}`, 500);
  }
}
async function getCustomizeComunity(req: Request, res: Response, next: NextFunction) {
  try {
    const user = req.user as userToken;
    const id = user.id;
    const customize = await customizeService.getCustomizeCommunity(id);
    respone(res, customize, "Get random component Successfully", 200);
  } catch (error) {
    respone(res, null, `${error}`, 500);
  }
